...
class BetTable(SQLModel, table=True):
    ...
    valid_amount: Decimal = Field(default=Decimal("0.00"), description="有效投注额", max_digits=15, decimal_places=2)
    ...
    rebate: Decimal = Field(default=Decimal("0.00"), description="回水金额", max_digits=15, decimal_places=2)
...
class MemberProfileTable(SQLModel, table=True):
    ...
    password: Optional[str] = Field(None, description="登录密码（bcrypt加密）")
