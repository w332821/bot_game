...
```bash
GET /admin/yueliao-users/search/phone?phone=13800138000
```
...
**接口地址**: `GET /admin/yueliao-users/search/yueliao-id`
...
```bash
GET /admin/yueliao-users/search/yueliao-id?yueliao_id=12345678
```
...
**接口地址**: `GET /admin/yueliao-users/statistics`
...
```bash
GET /admin/yueliao-users/statistics
```
...
```javascript
// 获取用户列表
axios.get('/admin/yueliao-users/list', {
  params: {
    page: 1,
    page_size: 20,
    search: '张三'
...
// 获取用户详情
axios.get(`/admin/yueliao-users/detail/${userId}`, {
  headers: {
    'Authorization': `Bearer ${token}`
  }
})
...
// 手机号搜索
axios.get('/admin/yueliao-users/search/phone', {
